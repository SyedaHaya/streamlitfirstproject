...
if select == "Home":
    ...
    sixes=pdata["6s"].sum()
    innings=pdata["innings"].sum()
    player=pdata["Player"].nunique()
    high_score=pdata["High_score"].sum()
    avg=pdata["Ave"].sum()
    ...
    col8.metric(label="Total Innings",value=innings)
    col9.metric(label="Total Player",value=player)
    col10.metric(label="Total High_score",value=high_score)
    col11.metric(label="Total Avg",value=avg)
    col12.metric(label="Total Strike_rate",value=strike_rate)
